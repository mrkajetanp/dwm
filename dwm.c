...
centeredmaster(Monitor *m)
{
	...
	if (i < m->nmaster) {
		...
		resize(c, m->wx + mx + m->gap->gappx, m->wy + my + m->gap->gappx, mw - 2*c->bw - 2*m->gap->gappx,
		...
		if ((i - m->nmaster) % 2) {
			h = (m->wh - ety) * (c->cfact / lfacts) - m->gap->gappx;
			resize(c, m->wx + m->gap->gappx, m->wy + ety + m->gap->gappx, tw - 2*c->bw - m->gap->gappx,
				h - 2*c->bw - m->gap->gappx, 0);
			if(ety + HEIGHT(c) + m->gap->gappx < m->mh)
				ety += HEIGHT(c) + m->gap->gappx;
			lfacts -= c->cfact;
		} else {
			h = (m->wh - oty) * (c->cfact / rfacts) - m->gap->gappx;
			resize(c, m->wx + mx + mw, m->wy + oty + m->gap->gappx,
			       tw - 2*c->bw - m->gap->gappx, h - 2*c->bw - m->gap->gappx, 0);
...
bstackhoriz(Monitor *m)
{
	...
	for (i = ty = 0, my = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
		if (i < m->nmaster) {
			...
			resize(c, m->wx + m->gap->gappx, m->wy + my + m->gap->gappx, mw - 2*(c->bw + m->gap->gappx), h - 2*c->bw - m->gap->gappx, 0);
			if(my + HEIGHT(c) < m->mh)
				my += HEIGHT(c);
			mfacts -= c->cfact;
		} else {
			h = (m->wh - mh - ty) * (c->cfact / sfacts) - m->gap->gappx;
			resize(c, m->wx + m->gap->gappx, m->wy + mh + ty + m->gap->gappx, mw - 2*c->bw - 2*m->gap->gappx, h - (2*c->bw) - m->gap->gappx, 0);
			if(ty + HEIGHT(c) + m->gap->gappx < m->mh)
				ty += HEIGHT(c) + m->gap->gappx;
			sfacts -= c->cfact;
		}
}
...
fibonacci(Monitor *mon, int s) {
	...
	for(i = 0, c = nexttiled(mon->clients); c; c = nexttiled(c->next)) {
		...
		int gapx = (i > mon->nmaster) ? 0 : mon->gap->gappx;
		int gapy = (i > 2) ? 0 : mon->gap->gappx;

		int gapw = ((i > mon->nmaster) ? 1 : 2) * mon->gap->gappx;
		int gaph = ((i > 2) ? 1 : 2) * mon->gap->gappx;

		resize(c, nx + gapx, ny + gapy, nw - 2 * c->bw - gapw, nh - 2 * c->bw - gaph, False);
	}
}
